#!/bin/bash
ts=$(date +%s)
pip --quiet install httpx bs4 typer rich tabulate
...
curl -s -o /usr/local/bin/waitfor "https://raw.githubusercontent.com/qsoyq/shell/main/scripts/python/tool/waitfor.py?v=$ts"
