...
def main(ip: list[str] = typer.Argument(..., help="待测试 ip 列表"), count: int = typer.Option(4, "-c", "--count", help="ping次数")):
    ...
    with ThreadPoolExecutor() as pool:
        ...
        for index, item in enumerate(res_li):
            ...
            # 2000为丢包
            elapsed = [i.time_elapsed_ms for i in item if i.time_elapsed_ms < 2000]
            if elapsed:
            ...
            # min avg
            rows.append((_ip, int(rtt_min * 1000), int(rtt_avg * 1000), int(rtt_max * 1000), f"{item.packet_loss:.2f}"))
