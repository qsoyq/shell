...
/**
 * @typedef {Object} battery.HTTPResponse
...
/**
 * @typedef {function(Error|string|null, Object, string|null): void} battery.HTTPCallback
...
/**
 * @typedef {function(Object, battery.HTTPCallback): battery.HTTPResponse} battery.HTTPMethod
 */

/**
 * @typedef {Object} battery.HttpClient
 * @property {battery.HTTPMethod} get - 发送 GET 请求
 * @property {battery.HTTPMethod} post - 发送 POST 请求
 * @property {battery.HTTPMethod} put - 发送 PUT 请求
 * @property {battery.HTTPMethod} delete - 发送 DELETE 请求
 */

/** @type {battery.HttpClient} */
var $httpClient;
 ...
 * @returns {Promise<battery.HTTPResponse>} 返回一个 Promise，解析为包含 error、response 和 data 的对象
 * @throws {Error} 如果请求失败，Promise 会被拒绝并返回错误信息
 */
async function request(method, params) {
    return new Promise((resolve, reject) => {
        /** @type {battery.HTTPMethod} */
 ...
 * 请求封装
 * @param {object} params
 * @returns {Promise<battery.HTTPResponse>}
 */
...
async function main() {
    try {
        let object = {}
        let key = mustGetScriptArgument("key")
        let url = `https://p.19940731.xyz/api/store/memory/${key}`
        let resp = await get(url)
        if (resp.error) {
            console.log(`获取设备信息失败, ${resp.error}`)
            ...
            if (data) {
                ...
                if (body?.state?.content) {
                    ...
                    object['icon'] = getScriptArgument('icon') || "https://cdn.jsdelivr.net/gh/Koolson/Qure@master/IconSet/Color/Apple.png"
                    object["title"] = `${body["state"]["content"]["hostname"]}`
                    object["content"] = content
                    object["interval"] = getScriptArgument("interval") || 300
                }
                return $done(object)
            }
