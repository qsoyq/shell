/** @namespace tastien.point */

/**
 * @typedef {Object} tastien.point.HTTPResponse
...
/**
 * @typedef {function(Error|string|null, Object, string|null): void} tastien.point.HTTPCallback
...
/**
 * @typedef {function(Object, tastien.point.HTTPCallback): tastien.point.HTTPResponse} tastien.point.HTTPMethod
 */

/**
 * @typedef {Object} tastien.point.HttpClient
 * @property {tastien.point.HTTPMethod} get - 发送 GET 请求
 * @property {tastien.point.HTTPMethod} post - 发送 POST 请求
 * @property {tastien.point.HTTPMethod} put - 发送 PUT 请求
 * @property {tastien.point.HTTPMethod} delete - 发送 DELETE 请求
 */

/** @type {tastien.point.HttpClient} */
var $httpClient;
 ...
 * @returns {Promise<tastien.point.HTTPResponse>} 返回一个 Promise，解析为包含 error、response 和 data 的对象
 * @throws {Error} 如果请求失败，Promise 会被拒绝并返回错误信息
 */
async function request(method, params) {
    return new Promise((resolve, reject) => {
        /** @type {tastien.point.HTTPMethod} */
 ...
 * 请求封装
 * @param {object} params
 * @returns {Promise<tastien.point.HTTPResponse>}
 */
...
async function main() {
    ...
    while (count <= 15) {
        ...
        if (msg && msg.includes("请勿重复提交")) {
            console.log(`${now} [DEBUG] [Exit] ${today} 请勿重复提交`)
            notificationPost("塔斯汀积分兑换", "兑换失败", `请勿重复提交`)
            return
        }

        if (msg && msg.includes("当前活动领取数量已达上限")) {
            console.log(`${now} [DEBUG] [Exit] ${today} 当前活动领取数量已达上限`)
            notificationPost("塔斯汀积分兑换", "兑换失败", `当前活动领取数量已达上限`)
            return
        }
