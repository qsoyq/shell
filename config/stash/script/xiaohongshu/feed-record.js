...
/** @namespace xiaohongshu.feed.record */

/**
 * @typedef {Object} xiaohongshu.feed.record.HTTPResponse
...
/**
 * @typedef {function(Error|string|null, Object, string|null): void} xiaohongshu.feed.record.HTTPCallback
...
/**
 * @typedef {function(Object, xiaohongshu.feed.record.HTTPCallback): xiaohongshu.feed.record.HTTPResponse} xiaohongshu.feed.record.HTTPMethod
 */

/**
 * @typedef {Object} xiaohongshu.feed.record.HttpClient
 * @property {xiaohongshu.feed.record.HTTPMethod} get - 发送 GET 请求
 * @property {xiaohongshu.feed.record.HTTPMethod} post - 发送 POST 请求
 * @property {xiaohongshu.feed.record.HTTPMethod} put - 发送 PUT 请求
 * @property {xiaohongshu.feed.record.HTTPMethod} delete - 发送 DELETE 请求
 */

/** @type {xiaohongshu.feed.record.HttpClient} */
var $httpClient;
 ...
 * @returns {Promise<xiaohongshu.feed.record.HTTPResponse>} 返回一个 Promise，解析为包含 error、response 和 data 的对象
 * @throws {Error} 如果请求失败，Promise 会被拒绝并返回错误信息
 */
async function request(method, params) {
    return new Promise((resolve, reject) => {
        /** @type {xiaohongshu.feed.record.HTTPMethod} */
 ...
 * 请求封装
 * @param {object} params
 * @returns {Promise<xiaohongshu.feed.record.HTTPResponse>}
 */
...
async function main() {
    ...
    if (!body) {
        console.log(`无法获取请求响应体`)
        return
    }

    let data = parseJsonBody(body)
    for (const item of data.data) {
        console.log(`user: ${item?.user?.nickname}, type: ${item?.type}, title: ${item?.display_title}, is_ads: ${item?.is_ads}, recommend: ${item?.recommend?.category_name}`)
