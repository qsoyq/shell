...
/**
 * @typedef {Object} v2ex.topics.HTTPResponse
...
/**
 * @typedef {function(Error|string|null, Object, string|null): void} v2ex.topics.HTTPCallback
...
/**
 * @typedef {function(Object, v2ex.topics.HTTPCallback): v2ex.topics.HTTPResponse} v2ex.topics.HTTPMethod
 */

/**
 * @typedef {Object} v2ex.topics.HttpClient
 * @property {v2ex.topics.HTTPMethod} get - 发送 GET 请求
 * @property {v2ex.topics.HTTPMethod} post - 发送 POST 请求
 * @property {v2ex.topics.HTTPMethod} put - 发送 PUT 请求
 * @property {v2ex.topics.HTTPMethod} delete - 发送 DELETE 请求
 */

/** @type {v2ex.topics.HttpClient} */
var $httpClient;
 ...
 * @returns {Promise<v2ex.topics.HTTPResponse>} 返回一个 Promise，解析为包含 error、response 和 data 的对象
 * @throws {Error} 如果请求失败，Promise 会被拒绝并返回错误信息
 */
async function request(method, params) {
    return new Promise((resolve, reject) => {
        /** @type {v2ex.topics.HTTPMethod} */
 ...
 * 请求封装
 * @param {object} params
 * @returns {Promise<v2ex.topics.HTTPResponse>}
 */
...
async function main() {
    ...
    let nodeList = mustGetScriptArgument("nodeList")
    let token = mustGetScriptArgument("token")
    let debug = getScriptArgument("debug")
    let now = getLocalDateString()

    let querystringArray = []
    nodeList.forEach((/** @type {any} */ element) => {
        querystringArray.push(`node=${element}`)
    })
    let qs = querystringArray.join("&")
    let res = await get({ url: `https://p.19940731.xyz/api/v2ex/topics?${qs}`, headers: { "content-type": "application/json", "Authorization": token } })
    if (res.error || res.response.status >= 400) {
        throw `${now} [Error] fetch topics error: ${res.error}, ${res.response.status}, ${res.data}`
    }

    let body = parseJsonBody(res.data)
    if (!body) {
        throw `invalid data: ${res.data}`
    }
    body.data.forEach((/** @type {{ topics: any[]; node: any; }} */ group) => {
        group.topics = group.topics.filter(topic => {
            topic.node = group.node
            let force = getScriptArgument("force")
            let keyname = `v2ex-topics-${topic.id}`
            let cache = getPersistentArgument(keyname)
            if (force || !cache) {
                return true
            }
            if (debug) {
                console.log(`${now}[Cache] skip ${topic.title}`)
            }
            return false
        })
    });
    ...
    for (const group of body.data) {
        for (const topic of group.topics) {
            ...
            let keyname = `v2ex-topics-${topic.id}`
            writePersistentArgument(keyname, now)
        }
    }
}
